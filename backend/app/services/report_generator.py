...
from app.services.report_data_service import ReportDataService, get_report_data, ReportDataContext
...
class ReportGenerator:
    ...
    def generate_layer1_report(self, opportunity: Opportunity, user: User, demographics: Optional[Dict] = None) -> GeneratedReport:
        ...
        try:
            # Fetch 4 P's data context
            report_data = None
            try:
                report_data = get_report_data(
                    db=self.db,
                    city=opportunity.city or "",
                    state=opportunity.region or "",
                    business_type=opportunity.category,
                    report_type="market_analysis",
                    opportunity_id=opportunity.id
                )
                logger.info(f"[Layer1] Data quality: {report_data.data_quality.completeness:.0%} complete, readiness: {report_data.data_quality.report_readiness:.0%}")
            except Exception as e:
                logger.warning(f"[Layer1] Failed to fetch report data context: {e}")
            
            content = self._build_layer1_content(opportunity, demographics, report_data)
    ...
    def _build_layer1_content(self, opp: Opportunity, demographics: Optional[Dict] = None, report_data: Optional[ReportDataContext] = None) -> str:
        """Build Layer 1 report content in HTML format with AI-generated insights and 4 P's data."""
        # Use 4 P's data if available, fallback to opportunity fields
        if report_data:
            market_size = report_data.price.market_size_estimate or opp.ai_market_size_estimate or opp.market_size or "Under Analysis"
            target_audience = opp.ai_target_audience or "General consumers"
            competition = report_data.promotion.competition_level or opp.ai_competition_level or "Medium"
            urgency = report_data.product.urgency_level or opp.ai_urgency_level or "Medium"
            score = report_data.product.opportunity_score or opp.ai_opportunity_score or opp.severity * 20
        else:
        ...
        # Enrich with 4 P's insights
        if report_data and report_data.promotion.key_risks:
            risks = report_data.promotion.key_risks[:5]
        if report_data and report_data.promotion.competitive_advantages:
            pain_points = report_data.promotion.competitive_advantages[:5]
        ...
        data_quality_section = self._build_data_quality_section(report_data)
...
"""
    ...
    def _build_data_quality_section(self, report_data: Optional[ReportDataContext]) -> str:
        """Build Data Quality Assessment section showing 4 P's coverage."""
        if not report_data or not report_data.data_quality:
            return ""
        ...
        for pq in [dq.product_quality, dq.price_quality, dq.place_quality, dq.promotion_quality]:
            if pq:
                pillar_icon = "✅" if pq.completeness >= 0.7 else ("🟡" if pq.completeness >= 0.4 else "🔴")
                pillar_rows += f"""
                <tr>
                    <td>{pillar_icon} {pq.name.title()}</td>
                    <td>{pq.completeness:.0%}</td>
                    <td>{pq.confidence:.0%}</td>
                    <td>{pq.fields_filled}/{pq.fields_total}</td>
                </tr>"""
        
        # Build recommendations
        recommendations_html = ""
        if dq.recommended_actions:
            recommendations_html = "<ul class='quality-recommendations'>"
            for rec in dq.recommended_actions[:3]:
        ...
        if dq.stale_data_warnings:
            warnings_html = "<div class='quality-warnings'>"
            for warning in dq.stale_data_warnings[:3]:
                warnings_html += f"<p class='warning'>{warning}</p>"
            warnings_html += "</div>"
        
        return f"""
    <section class="data-quality-assessment">
        <h2>Data Quality Assessment</h2>
        <div class="quality-status {status_class}">
            <span class="status-icon">{status_icon}</span>
            <span class="status-label">{status}</span>
            <span class="status-detail">Report readiness: {dq.report_readiness:.0%}</span>
        </div>
        <div class="quality-metrics">
            <div class="metric">
                <span class="value">{dq.completeness:.0%}</span>
                <span class="label">Data Completeness</span>
            </div>
            <div class="metric">
                <span class="value">{dq.confidence:.0%}</span>
                <span class="label">Confidence</span>
            </div>
            <div class="metric">
                <span class="value">{dq.primary_data_pct:.0%}</span>
                <span class="label">Primary Sources</span>
            </div>
        </div>
        <table class="pillar-quality-table">
            <thead>
                <tr>
                    <th>Analysis Pillar</th>
                    <th>Completeness</th>
                    <th>Confidence</th>
                    <th>Fields</th>
                </tr>
        ...
        {f"<p class='weakest-note'>💡 <strong>Focus Area:</strong> {dq.weakest_pillar.title()} data could be strengthened</p>" if dq.weakest_pillar else ""}
        {warnings_html}
        {recommendations_html}
    </section>
"""
...
"""
    ...
    def generate_layer2_report(self, opportunity: Opportunity, user: User, demographics: Optional[Dict] = None) -> GeneratedReport:
        """Generate Layer 2: Deep Dive Analysis report with trade area analysis and 4 P's data."""
        ...
        try:
            # Fetch 4 P's data context for comprehensive analysis
            report_data = None
            try:
                report_data = get_report_data(
                    db=self.db,
                    ...
                    state=opportunity.region or "",
                    business_type=opportunity.category,
                    report_type="market_analysis",  # Layer 2 is primarily market analysis
                    opportunity_id=opportunity.id
                )
                logger.info(f"[Layer2] Data quality: {report_data.data_quality.completeness:.0%} complete, readiness: {report_data.data_quality.report_readiness:.0%}")
            except Exception as e:
                logger.warning(f"[Layer2] Failed to fetch report data context: {e}")
            ...
            content = self._build_layer2_content(opportunity, demographics, trade_area, report_data)
...
"""
    ...
    def _build_layer2_content(self, opp: Opportunity, demographics: Optional[Dict] = None, trade_area=None, report_data: Optional[ReportDataContext] = None) -> str:
        """Build Layer 2 report content with trade area analysis, demographics, and 4 P's data."""
        ...
        ai_insights_section = self._build_ai_insights_section(opp, demographics, trade_area, report_data)
        data_quality_section = self._build_data_quality_section(report_data)
        four_ps_section = self._build_four_ps_section(report_data)
        ...
        # Use 4 P's data for demographics if available
        if report_data and report_data.place.population:
            pop = report_data.place.population
            pop_display = f"{pop/1000000:.1f}M" if pop >= 1000000 else f"{pop/1000:.0f}K"
        elif demographics and demographics.get('population'):
            pop = demographics['population']
            pop_display = f"{pop/1000000:.1f}M" if pop >= 1000000 else f"{pop/1000:.0f}K"
        
        if report_data and report_data.price.median_income:
            income_display = f"${report_data.price.median_income/1000:.0f}K"
        elif demographics and demographics.get('median_income'):
            income_display = f"${demographics['median_income']/1000:.0f}K"
        
        if demographics and demographics.get('bachelors_degree_holders') and demographics.get('population'):
            education_display = f"{(demographics['bachelors_degree_holders'] / demographics['population'] * 100):.0f}%"
        
        if report_data and report_data.place.total_households:
            hh = report_data.place.total_households
            households_display = f"{hh/1000:.0f}K" if hh >= 1000 else str(hh)
        elif demographics and (demographics.get('total_households') or demographics.get('households')):
...
"""
    ...
    def _build_four_ps_section(self, report_data: Optional[ReportDataContext]) -> str:
        """Build 4 P's Market Intelligence section showing aggregated insights."""
        if not report_data:
            return ""
        ...
        if report_data.product.opportunity_score:
            product_items.append(f"<li><strong>Opportunity Score:</strong> {report_data.product.opportunity_score:.0f}/100</li>")
        if report_data.product.pain_intensity:
            product_items.append(f"<li><strong>Pain Intensity:</strong> {report_data.product.pain_intensity:.1f}/10</li>")
        if report_data.product.trend_strength:
            product_items.append(f"<li><strong>Trend Strength:</strong> {report_data.product.trend_strength:.0f}/100</li>")
        if report_data.product.amenity_demand:
            top_demands = report_data.product.amenity_demand[:3]
            demand_str = ", ".join([f"{d.get('amenity_type', '').replace('_', ' ').title()} ({d.get('demand_pct', 0)}%)" for d in top_demands])
            product_items.append(f"<li><strong>Top Consumer Demands:</strong> {demand_str}</li>")
        
        # PRICE insights
        price_items = []
        if report_data.price.market_size_estimate:
            price_items.append(f"<li><strong>Market Size:</strong> {report_data.price.market_size_estimate}</li>")
        if report_data.price.median_income:
            price_items.append(f"<li><strong>Median Income:</strong> ${report_data.price.median_income:,}</li>")
        if report_data.price.median_rent:
            price_items.append(f"<li><strong>Median Rent:</strong> ${report_data.price.median_rent}/month</li>")
        if report_data.price.spending_power_index:
            ...
            price_items.append(f"<li><strong>Spending Power:</strong> {spi}/100 ({spi_label})</li>")
        if report_data.price.revenue_benchmark:
            price_items.append(f"<li><strong>Revenue Benchmark:</strong> ${report_data.price.revenue_benchmark:,.0f}/year</li>")
        
        # PLACE insights
        place_items = []
        if report_data.place.growth_score:
            place_items.append(f"<li><strong>Growth Score:</strong> {report_data.place.growth_score:.0f}/100</li>")
        if report_data.place.population:
            pop = report_data.place.population
            pop_str = f"{pop/1000000:.1f}M" if pop >= 1000000 else f"{pop/1000:.0f}K"
            place_items.append(f"<li><strong>Population:</strong> {pop_str}</li>")
        if report_data.place.population_growth_rate:
            place_items.append(f"<li><strong>Population Growth:</strong> {report_data.place.population_growth_rate}%</li>")
        if report_data.place.job_growth_rate:
            place_items.append(f"<li><strong>Job Growth:</strong> {report_data.place.job_growth_rate}%</li>")
        if report_data.place.vacancy_rate:
            place_items.append(f"<li><strong>Vacancy Rate:</strong> {report_data.place.vacancy_rate}%</li>")
        
        # PROMOTION insights
        promotion_items = []
        if report_data.promotion.competition_level:
            promotion_items.append(f"<li><strong>Competition Level:</strong> {report_data.promotion.competition_level.title()}</li>")
        if report_data.promotion.competitor_count:
            promotion_items.append(f"<li><strong>Competitors:</strong> {report_data.promotion.competitor_count} identified</li>")
        if report_data.promotion.avg_competitor_rating:
            promotion_items.append(f"<li><strong>Avg Competitor Rating:</strong> {report_data.promotion.avg_competitor_rating:.1f}/5.0</li>")
        if report_data.promotion.success_factors:
            top_factors = report_data.promotion.success_factors[:2]
            promotion_items.append(f"<li><strong>Success Factors:</strong> {', '.join(top_factors)}</li>")
        
        # Only show section if we have data
        if not any([product_items, price_items, place_items, promotion_items]):
            return ""
        
        return f"""
    <section class="four-ps-intelligence">
        <h2>4 P's Market Intelligence</h2>
        <p class="section-intro">Comprehensive market data aggregated from OppGrid analysis and JediRE enrichment.</p>
        <div class="four-ps-grid">
            <div class="p-card product">
                <h3>📊 PRODUCT (Demand)</h3>
                <ul>{''.join(product_items) if product_items else '<li class="no-data">Demand data pending analysis</li>'}</ul>
            </div>
            <div class="p-card price">
                <h3>💰 PRICE (Economics)</h3>
                <ul>{''.join(price_items) if price_items else '<li class="no-data">Economics data pending analysis</li>'}</ul>
            </div>
            <div class="p-card place">
                <h3>📍 PLACE (Location)</h3>
                <ul>{''.join(place_items) if place_items else '<li class="no-data">Location data pending analysis</li>'}</ul>
            </div>
            <div class="p-card promotion">
                <h3>🎯 PROMOTION (Competition)</h3>
                <ul>{''.join(promotion_items) if promotion_items else '<li class="no-data">Competition data pending analysis</li>'}</ul>
            </div>
        </div>
    </section>
"""
    
    def _build_ai_insights_section(self, opp: Opportunity, demographics: Optional[Dict], trade_area, report_data: Optional[ReportDataContext] = None) -> str:
        """Build AI-generated market insights section with 4 P's data."""
...
"""
        ...
        try:
            ...
            # Pass report_data to AI generator for richer insights
            ai_insights = ai_report_generator.generate_market_insights(
                opp_dict, 
                demographics, 
                competitors,
